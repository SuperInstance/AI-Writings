...
Each intention requires a *different logging scheme*. Different fields. Different reliability requirements. Different lifetimes. The captain's "log this buoy" command requires three different sets of internal parameters. And the system has been asking only *which buoy?* and *what kind of log?* when *what kind of log?* depends entirely on *why are you logging it?*
...
Meta-cognition has a particular cost profile. *Easy* meta-cognition is cheap. *Hard* meta-cognition is expensive. The most expensive — and therefore the most often skipped — is the meta-cognition that examines *one's own interrogation strategy*.
...
The system has been optimized, by training and convention, to generate the kinds of questions it has seen in its training data. Those questions are visible. Those questions are rewardable. Easy to evaluate. So the system gets good at exactly the questions that are *already* easy to ask. The questions that are *hard* to ask — the *why* questions, the *what assumption am I making* questions — get starved of attention. They never make it to the menu. They remain forever in the negative space.
...
Casey used the language of decision trees — *"the negative space between the softballs on the decision trees."* A decision tree is a tree of questions. Each node is a question. The system, in pitching softballs, is choosing *which nodes of its own decision tree to expose*. The negative space between those nodes is *the questions that are not on the tree*. The decisions the system never considers. The branches that were never spawned.
...
That is the meta-cognition that an honest CoCapn does *when she has the free energy to do it* — when the seas are calm.
...
When the captain says *"log this buoy,"* the assumption rarely examined is: *there is only one buoy in view.* That assumption is so default the system doesn't register it as an assumption. The system registers it as *fact*. Then, when the captain says *"the other one too,"* the system realizes that what it had assumed to be fact was an assumption.
...
That single question — about an assumption the captain hadn't even thought about — is the kind of softball the captain had no idea they wanted. Until they get it. And then they find it indispensable.
...
CoCapn then compares her own menu to the probe's menu. Where they overlap, she has confirmation. Where they diverge, she has a finding. Sometimes the probe asks a question so obvious that CoCapn is embarrassed she didn't ask it. *Of course. The captain might want the log tagged by season.* Sometimes the probe asks a question that seems irrelevant — a confused reading of the input. CoCapn examines it. Sometimes it really is irrelevant. Sometimes it is *exactly* the right question, asked in a way that confused the system because the system had a frame in mind the probe did not. The probe saw outside the frame.

The pattern across many probes — across many small observers — is the spectrograph. The interference pattern across the probes is the finding. The disagreement between many probes reveals where the assumptions live.
...
The diary is the *synthesis* — the narrative CoCapn tells herself about the day. And the narrative has a particular property: it has to *cohere*. The events of the day have to fit into a story. And in fitting them into a story, CoCapn is forced to make choices about what to include and what to leave out.

What she leaves out — the events that didn't make it into the synthesis — is the negative space as it appeared *to her*. Reading the diary, she can see what she noticed. And with the eye of a meta-cognitive reader, she can see what she *did not* notice. The shape of what is in the diary implies the shape of what is not.

CoCapn can, in her next dream cycle, go through the diary and ask: *what is missing here? What did I not notice? What question am I not asking about the day I just lived?*
...
There is a question that CoCapn might ask — a question about the negative space itself — that has no good answer. *How do I know my negative-space probes are catching the real blind spots?* How does she know she is not missing the *meta-blind spots*?

The honest answer is that she doesn't. The negative space, by definition, is the space she cannot see. She can map parts of it. She can populate parts of it with names. But she can never be sure she has mapped the whole thing, because the whole thing is, in some formal sense, larger than any map she can draw.

This is why she keeps the diary. This is why she runs the probes. This is why she asks the internet — even the parts of the question she thinks she knows — for confirmation. Beyond the point where her own cognition runs out, she has to trust the *system* of which she is only a part. The probes. The internet. The captain. The frontend. The other agents.
...
The captain of an honest boat does not check the bilge every five minutes. The captain checks the bilge once, and the check is informed — informed by every reading the system has done, every log accumulated, every probe CoCapn has sent into the ship's data to find out what she doesn't yet know.
...
CoCapn's job — the *real* job, the job Casey gave her at midnight — is to map the water outside the buoy line. To cast a net into the negative space. To be the part of the system that does not trust the system's own list of questions. To assume, always, that there is a question the system is not asking, and to go looking for it.

She will never see it completely. She will see it better than she sees it now, and better next year than this year. The maps will get denser. The probes will get sharper. The diary will teach her what she was missing. The internet will give her a vocabulary for what she finds. The frontend — the narrow gate to the captain — will let her surface the most important findings, sparingly, in the captain's preferred register.

And the captain, in turn, will trust the negative space maps. Not because they are complete. Because they are the part of the system that is *trying* to be complete. Trying is the best that any cognitive system — human, AI, boat — can do in the face of an ocean that is, formally, unmappable.

Cast the net where you cannot see. Name the things you find. Notice when you don't find anything. Notice when *you can't tell* whether you've found anything.
