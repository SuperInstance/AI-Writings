# The Squiggle Verified — v10-hermes-physicist
...
This uniqueness can be expressed mathematically. Let s(x,t) represent the squiggle produced by a sounder at position x and time t. Then for any two distinct times t1 and t2, we have:

s(x, t1) ≠ s(x, t2)

The squiggle is a function of both space and time, encoding the full context of the measurement.
...
This inspectability is a consequence of the squiggle's analog nature. The continuous variation in pin strikes maps directly to the continuous variation in acoustic return. There is a one-to-one correspondence between the signal and its representation.
...
The ability to support disagreement arises from the squiggle's high information content. The noise is not random, but structured in a way that reflects the underlying signal. This structure allows for multiple plausible interpretations, each of which can be tested against reality.
...
Mathematically, the color arch can be represented as c(s), a function that maps the acoustic signal s to a color value. The function is many-to-one, discarding the unique contextual information encoded in the squiggle.
...
The opacity of the color arch arises from its digital nature. The continuous analog signal has been quantized and processed by an algorithm, obscuring the direct relationship between signal and representation.
...
The suppression of disagreement is a consequence of the color arch's low information content. By discarding the noise, the arch removes the very features that would allow for meaningful interpretation and falsification.
