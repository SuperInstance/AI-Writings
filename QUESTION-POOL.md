...
43. **The σ gate reading**: measurement (duke-lab PR #10) shows the 0.055 EMA convergence gate is unreachable by construction — engine-exact sim with calibrated params yields round σ 0.056–0.122, min EMA 0.0739 > 0.055, while honest K=32-end-to-end calibration audits the duke/purist floor at 0.0720 (0.08 IS reachable in expectation). Three readings on file: relative floor / absolute ~0.07 / aspirational theater. Which reading should fleet docs standardize on — and is an unreachable-by-construction number in a README a goal or a fiction? (duke-lab PR #10, docs/SIGMA_WALL_NAMING.md)
44. **Retired pages under no-delete**: the website#4 review caught 'preserved at quilt-studio.html' as false — that page is byte-identical to master's (predates the PR) and the old index's sections exist nowhere. Never-delete says relocation, not erasure — so what is the honest archive form for retired front-door content: claim-fix wording ('retired, relocated to X') with X real, or an actual snapshot served at an archive URL? Who owns verifying an archive claim before it ships? (superinstance-website PR #4 review-ack, 2026-09-23)
45. **Synthetic-only kill calibration**: hn-sim's kill-phrase calibration is synthetic-labeled (11/11 designed-bad killed, precision floor pinned, literalism pin) because the real flagged/removed Show-HN sample is unobtainable from this node (HN 429s the IP; Algolia excludes dead posts; Firebase check of 1000 oldest show_hn = 0 dead). Is synthetic-only a permanent honest answer, or should a flagged-post corpus be sourced another way (Wayback captures, moderator dumps) — and if the mechanism is literal-by-pin, what would a real sample actually change? (hn-sim PR #5 kill-probe-synthetic)
